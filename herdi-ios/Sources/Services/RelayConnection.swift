...
final class RelayConnection {
    static let defaultRelayURL = "ws://100.108.73.69:8375"
    ...
    init() {
        startPathMonitor()
        connect(to: Self.defaultRelayURL)
    }
    ...
    private func listen() {
        task?.receive { [weak self] result in
            ...
            case .failure:
                DispatchQueue.main.async {
                    self?.lastError = "WebSocket receive failed"
    ...
    private func handle(_ text: String) {
        ...
        DispatchQueue.main.async { [self] in
            ...
            case "agents":
                ...
                let defaults = UserDefaults(suiteName: "group.com.dawson.herdi")
            ...
            case "pane_content":
                if let pid = msg.pane_id, let content = msg.content ?? msg.prompt {
