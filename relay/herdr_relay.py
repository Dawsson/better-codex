...
HOST = os.environ.get("HERDR_RELAY_HOST", "0.0.0.0")
...
async def main():
    ...
    server = await serve(handle_client, HOST, WS_PORT, process_request=process_request)
    hosts = ["local"] + REMOTES
    print(f"herdr-remote relay on {HOST}:{WS_PORT} (WebSocket + HTTP POST)")
